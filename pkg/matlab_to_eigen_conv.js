 ...
 * @param {string} src
 * @param {string} types
 * @param {boolean} gen_mex_wrapper
 * @returns {TranspilerOutput}
 */
export function transpile_wrap(src, types, gen_mex_wrapper) {
    const ptr0 = passStringToWasm0(src, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ptr1 = passStringToWasm0(types, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len1 = WASM_VECTOR_LEN;
    const ret = wasm.transpile_wrap(ptr0, len0, ptr1, len1, gen_mex_wrapper);
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('matlab_to_eigen_conv_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
